...
class XDPExampleTestCase(unittest.TestCase):
    ...
    def _xdp_test_run(self, given_packet, expected_packet, expected_return):
        ...
        repeat = 1
        ret = libbcc.lib.bpf_prog_test_run(self.bpf_function.fd, repeat, ctypes.byref(given_packet), size, ctypes.byref(packet_output), ctypes.byref(packet_output_size), ctypes.byref(test_retval), ctypes.byref(duration))
    ...
    def setUp(self):
        bpf_prog = BPF(src_file="program.c")
    ...
    def test_transform_dst(self):
        ...
        expected_packet = Ether(dst='00:e0:72:59:71:1d') / IP() / TCP(dport=9090)
